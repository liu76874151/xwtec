...
var infoComponent = {
		...
		getTreeData : function()
		{
			...
			$.singleReq({
				...
				{
					"reqUrl" : "content",
					"reqMethod" : "querryChannelByChannal",
				...
				success : function(ret)
				{
					if(ret){
						if(ret.retCode == GLOBAL_INFO.SYS_SUCCESS){
							if(ret.retObj){
								custemInfoTree.loadJSONObject(ret.retObj.root);
							}
						}else{
							if(ret.resMsg){
		...
		tonclick : function(id) {
			var chanId=custemInfoTree.getUserData(id, "CHANID");
			var parentId=custemInfoTree.getUserData(id, "PARENTID");
			var type=custemInfoTree.getUserData(id, "TYPE");
			var channelNum=$("#channelNum").val();
			var title = custemInfoTree.getItemText(id);
			var chanUrl = GLOBAL_INFO.CONTEXTPATH + "/page/content/channel/channel_list.jsp?parentId="+parentId+"&chanId="+chanId+"&channelNum="+channelNum+"&type="+type;
			var docUrl = GLOBAL_INFO.CONTEXTPATH + "/page/content/document/doc_list.jsp?parentId="+parentId+"&chanId="+chanId+"&type="+type;
			var url2 = GLOBAL_INFO.CONTEXTPATH + "/page/content/channel/channel_list.jsp?parentId="+parentId+"&chanId="+chanId+"&channelNum="+channelNum;
			if(!frameTabbar.cells(id)){
				frameTabbar.clearAll();//--清空tabbar
				frameTabbar.addTab(id, title+"--内容管理", "230px");
				frameTabbar.addTab(id+"1", title+"--频道管理", "230px");
		
				//frameTabbar.addTab(id+"2", title+"--内容发布", "230px");
				
			    frameTabbar.setContentHref(id, docUrl);
			    frameTabbar.setContentHref(id+"1", chanUrl);
				//frameTabbar.setContentHref(id+"2", url2);
